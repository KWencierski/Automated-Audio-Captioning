...
class HDFDatamodule(LightningDataModule):
    def __init__(
        ...
        root: str | Path,
        pretrain_hdfs: str | Iterable[str] = (),
        ...
        root = Path(root)
        if isinstance(pretrain_hdfs, str):
            pretrain_hdfs = [pretrain_hdfs]
        else:
            pretrain_hdfs = list(pretrain_hdfs)
        ...
        self.root = root
        self.pretrain_hdfs = pretrain_hdfs
    ...
    def train_transform(self, item: dict[str, Any]) -> dict[str, Any]:
        refs = item["captions"]
        captions = self.tokenizer.encode_batch(refs, disable_unk_token=True)
    ...
    def _common_transform(
        ...
        if "captions" in item:
            ...
            captions = torch.as_tensor([cap.ids for cap in captions])  # (5, n_words)
    ...
    def setup_train(self) -> None:
        ...
        print("train_collate_fn", train_collate_fn)
