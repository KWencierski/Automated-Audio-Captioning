...
class AACTokenizer:
    ...
    def default_tokenizer(
        ...
        model = BPE(initial_vocab, merges=[], unk_token=unk_token)
    ...
    def _create_merges(cls, vocab: dict[str, int]) -> list[str]:
        merges = []
        for token in vocab:
            if token.startswith("##"):
                continue
            for i in range(len(token)):
                merges.append(f"{token[:i]} {token[i:]}")
        return merges
    ...
    def train_from_iterator(
        ...
        if trainer is None:
            trainer = BpeTrainer(special_tokens=self.special_tokens)  # type: ignore
